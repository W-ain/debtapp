...
if (!$error_message) {
    try {
        ...
        if (!$error_message) {
            // 承認状態を更新
            $update = $pdo->prepare(
                "UPDATE debts SET verified = 1, token = NULL WHERE token = ?"
            );
            ...
            try {
                ...
                $error_message .=
                    " <br>借主への通知メール送信エラー: {$mail_borrower->ErrorInfo}";
            }
            ...
            try {
                ...
                $error_message .=
                    " <br>貸主への通知メール送信エラー: {$mail_lender->ErrorInfo}";
            }
        }
        ...
        <?php if ($success_message && !$error_message): ?>
            ...
            <div class="details" style="margin-top: 15px;">
                <p style="text-align: center; font-size: 14px;">
                    メール通知状況:
                    <br>借主（あなた）:
                    <?= $borrower_email_sent ? '✅ 送信済み' : '❌ 送信エラー' ?>
                    <br>貸主（<?= htmlspecialchars($debt['lender_name'] ?? '---') ?>）:
                    <?= $lender_email_sent ? '✅ 送信済み' : '❌ 送信エラー' ?>
                </p>
            </div>
